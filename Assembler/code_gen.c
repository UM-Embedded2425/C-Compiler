...
void codeGen()
{
    ...
    int code = 0, reg = 0, i = 0, seg = 0, rel = 0, warn = 0, i_3 = 1, last_filled_pos = 0;
    lc.value[CODE_SEGMENT] = 0;
    while (i < current_ir){

        if(IR[i].op_type != CSEG_OP) lc.value[CODE_SEGMENT] += IR[i].N;
        ...
        case CJNE_OP:
            ...
            if (IR[i].info == A_DIRECT)
            {
                code |= (0x0f & 0x05) << 16;
                code |= (0xff & IR[i].op_2) << 8;
                rel = (*IR[i].op_3 - lc.value[CODE_SEGMENT])/3;
                code |= (0xff & rel);
                warn = checkDirAddr(IR[i].op_2, lc.value[CODE_SEGMENT] - IR[i].N);
        ...
        {
            fprintf(output, "%02X", (code >> ((IR[i].N - 1 - i2) * 8)) & 0xFF);
            if (!((i_3) % 3))
                fprintf(output, ",\n");
            i_3++;
        }
        // Check for segment code
        if (seg)
        {
            lc.value[CODE_SEGMENT] += IR[i].N;
            // If negative, change the position on the file
            if (IR[i].N < 0)
            {
                fseek(output, (IR[i].N / 3) * 8, SEEK_CUR);
            }
            else
                for (size_t i2 = 0; i2 < IR[i].N; i2++)
                {
                    if ((lc.value[CODE_SEGMENT]- IR[i].N + i2) < last_filled_pos)
                    {
        ...
        if (lc.value[CODE_SEGMENT] > last_filled_pos)
            last_filled_pos = lc.value[CODE_SEGMENT];
        code = 0;
        i++;
    }
    if (warn)
        printf("There are warnings to verify.");
    fseek(output, -2, SEEK_CUR);
    fprintf(output, ";\n");
    fclose(output);
}